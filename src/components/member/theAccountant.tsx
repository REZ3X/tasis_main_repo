...
/**
 * TheAccountant Component
 * Displays a section showcasing the financial team members (accountants) of TASIS
 * Features a responsive layout with background gradients and hover effects
 */
export default function TheAccountant() {
  const accountants = [
    {
      name: "Hosea Ega R. S.",
      role: "Bendahara 1",
    ...
    <section className="relative px-6 py-20 bg-[#0d1216] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-300 text-lg">Tim Keuangan TASIS</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-12">
          {accountants.map((accountant, index) => (
            <div
              key={index}
              className={`relative group ${accountant.order} transition-transform duration-300 hover:scale-105`}
            >
              ...
              <div className="text-center mt-4 space-y-2">
                ...
                <p className="text-gray-300">{accountant.role}</p>
              </div>
            </div>
          ))}
        </div>
